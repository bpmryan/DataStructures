...
public class Sorter implements GodricsHat {
    public void insertion(int[] array) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            int currentKey = i - 1; // compares current elements to the pervious element after substracting the
                                    // current index by 1
    ...
    public void merge(int[] array) {
         ...
         * then sort/compare every element in each section and group them back together
         * and repeat
    ...
    public void quick(int[] array, int p, int r) {
        ...
        if (p < r) { // base case for recursion
            ...
            // Part that the elements less than the pivot to the left and the greatest on
            // the right
    ...
    public void quickLoopy(int[] array) {
        ...
        // creates a stack as an array as a psuedo recursive function
        int[] stack = new int[array.length];

        // points to the top of the stack
        // at the moment is empty since there isn't an index that goes under 0
        int top = -1;

        // push initial array bounds [p, r] onto the stack
        // FILO stack order
        stack[++top] = p;
        stack[++top] = r;

        // loop runs while there are still subarrays to sort
        // because top = -1 and the condition is false, the loop will run 
        while (top >= 0) {
            r = stack[top--];
            p = stack[top--];

            // lines 146 - 170 works similarly to the recursion version
            // pivot is the last element
            int pivot = array[r];
            // traces the boundary elements <= pivot
            int i = p - 1;

            // loops through the subarray from p to r-1
            // j is the current index that is being compared to
            for (int j = p; j < r; j++) {
                if (array[j] <= pivot) {
                    i++;
            ...
            // pushes the elements left of the pivot to the stack to sort later
            if (partition - 1 > p) {
                stack[++top] = p;
                stack[++top] = partition - 1;
            }
            // pushes the elements right of the pivot to the stack
            if (partition + 1 < r) {
                stack[++top] = partition + 1;
                stack[++top] = r;
            }
